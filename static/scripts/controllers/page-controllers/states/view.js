(function () {
  "use strict";
  angular.module("app").controller("ViewStateCtrl", ViewStateCtrl);

  ViewStateCtrl.$inject = ["$scope", "$http", "$rootScope"];

  function ViewStateCtrl($scope, $http, $rootScope) {
    var vm = $scope;
    vm.table = {
      title: "State",
      limit: "10",
      fields: [
        ["State's Name", "name"],
        ["State's GDP (Crore Rs)", "gdp"],
        ["State's Population (Lakhs)", "population"],
    ...
    vm.getData = function () {
      ...
      $http.post("/api/location/get/state", payload).then(function (res) {
        if (res.data.success) {
