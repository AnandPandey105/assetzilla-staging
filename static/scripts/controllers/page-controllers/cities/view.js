(function () {
  "use strict";
  angular.module("app").controller("ViewCityCtrl", ViewCityCtrl);

  ViewCityCtrl.$inject = ["$scope", "$http", "$rootScope"];

  function ViewCityCtrl($scope, $http, $rootScope) {
    var vm = $scope;
    vm.table = {
      title: "City",
      limit: "10",
      fields: [
        ["City's Name", "name"],
        ["City's GDP (Crore Rs)", "gdp"],
        ["City's Population (Lakhs)", "population"],
    ...
    vm.getData = function () {
      ...
      $http.post("/api/location/get/city", payload).then(function (res) {
        if (res.data.success) {
    ...
    vm.getLocation = function (val) {
      return $http
        .post("/api/city/custom-typeahead", {
          data: val,
          status: "0",
        })
