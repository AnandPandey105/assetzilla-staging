(function () {
  ...
  angular
    ...
    .controller("AddCityCtrl", AddCityCtrl)
    .directive("fileModel", [
      "$parse",
      function ($parse) {
        return {
  ...
  AddCityCtrl.$inject = ["$scope", "$http", "$rootScope", "$location"];

  function AddCityCtrl($scope, $http, $rootScope, $location) {
    var vm = $scope;
    ...
    vm.add = function () {
      ...
      // if ((vm.data.images.Cities[0] == "")) { alert("Please Add atleast 1 Image"); vm.loader = false; return; }
      var fd = new FormData();
      vm.data.images.Cities.forEach((image) => {
      ...
      $http
        ...
        .then(
          function (res) {
            if (res.data.success) {
              vm.data.images.Cities = res.data.results;
              vm.data.banner_image = [
                "Cities",
                vm.data.images.Cities[vm.data.banner_image],
              ];
              $http.post("/api/city/add", vm.data).then(
                function (res) {
                ...
                function (error) {
                  vm.loader = false;
                  $rootScope.createError(
                    "Some error occured while adding City .."
                  );
                }
              );
            }
