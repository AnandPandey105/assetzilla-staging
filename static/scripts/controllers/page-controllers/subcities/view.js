(function () {
  "use strict";
  angular.module("app").controller("ViewSubcityCtrl", ViewSubcityCtrl);
  ViewSubcityCtrl.$inject = ["$scope", "$http", "$rootScope"];

  function ViewSubcityCtrl($scope, $http, $rootScope) {
    var vm = $scope;
    vm.table = {
      title: "Subcity",
      limit: "10",
      fields: [
        ["Subcity's Name", "name"],
        ["Subcity's GDP (Crore Rs)", "gdp"],
        ["Subcity's Population (Lakhs)", "population"],
    ...
    vm.getData = function () {
      ...
      $http.post("/api/location/get/subcity", payload).then(function (res) {
        if (res.data.success) {
    ...
    vm.getLocation = function (val) {
      return $http
        .post("/api/subcity/custom-typeahead", {
          data: val,
          status: "0",
        })
