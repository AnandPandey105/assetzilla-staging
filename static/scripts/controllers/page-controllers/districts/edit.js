(function () {
  ...
  angular
    ...
    .controller("EditDistrictCtrl", EditDistrictCtrl)
    .directive("fileModel", [
      "$parse",
      function ($parse) {
        return {
  ...
  EditDistrictCtrl.$inject = ["$scope", "$http", "$rootScope", "$location"];

  function EditDistrictCtrl($scope, $http, $rootScope, $location) {
    var vm = $scope;
    ...
    vm.set_data = function () {
      ...
      $http
        .post("/api/district/get-doc", { url: decodeURIComponent(url) })
        .then(function (res) {
          if (res.data.success) {
            vm.data = res.data.data;
            vm.new_banner_image = vm.data.images.Districts.indexOf(
    ...
    vm.imageUpload = (files, field) => {
      ...
      if (vm.isPasting && false) {
        ...
        $http
          .post("/api/district/summernoteMulter", fds, {
          ...
          .then(
            function (res) {
              ...
              res.data.results.forEach((i) => {
                // vm.data[field] += `<img src='https://assetzilla-bucket.s3.amazonaws.com/dyn-res/district/${i}' style="width:100%"/>`
                let url = `https://d13ir9awo5x8gl.cloudfront.net/dyn-res/district/${i}`;
    ...
    vm.edit = function () {
      ...
      if (vm.new_banner_image > vm.data.images.Districts.length + c - 1) {
        alert("Please Select a Banner Image");
        return;
      }
      if (c > 0) {
        $http
          .post("/api/district/multer", fd, {
            headers: { "Content-Type": undefined },
            transformRequest: angular.identity,
          })
          .then(
            function (res) {
              if (res.data.success) {
                res.data.results.forEach((element) => {
                  vm.data.images.Districts.push(element);
                });
                vm.data.banner_image = [
                  "Districts",
                  vm.data.images.Districts[vm.new_banner_image],
                ];
                $http.post("/api/district/edit", vm.data).then(
                  function (res) {
                    alert(res.data.message);
                    if (res.data.success) {
                      window.location = "/admin-panel#/app/district/view";
                    }
                  },
                  function (error) {
                    vm.loader = false;
                    $rootScope.createError(
                      "Some error occured while editing District .."
                    );
                  }
                );
              }
            ...
            function (error) {
              vm.loader = false;
              $rootScope.createError(
                "Some error occured while uploading District images .."
              );
            }
          );
      } else {
