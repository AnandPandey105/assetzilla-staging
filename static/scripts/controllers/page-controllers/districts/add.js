(function () {
  ...
  angular
    ...
    .controller("AddDistrictCtrl", AddDistrictCtrl)
    .directive("fileModel", [
      "$parse",
      function ($parse) {
        return {
  ...
  AddDistrictCtrl.$inject = ["$scope", "$http", "$rootScope", "$location"];

  function AddDistrictCtrl($scope, $http, $rootScope, $location) {
    var vm = $scope;
    ...
    vm.add = function () {
      ...
      // if ((vm.data.images.Districts[0] == "")) { alert("Please Add atleast 1 Image"); vm.loader = false; return; }
      var fd = new FormData();
      vm.data.images.Districts.forEach((image) => {
      ...
      $http
        ...
        .then(
          function (res) {
            if (res.data.success) {
              vm.data.images.Districts = res.data.results;
              vm.data.banner_image = [
                "Districts",
                vm.data.images.Districts[vm.data.banner_image],
              ];
              $http.post("/api/district/add", vm.data).then(
                function (res) {
                ...
                function (error) {
                  vm.loader = false;
                  $rootScope.createError(
                    "Some error occured while adding District .."
                  );
                }
              );
            }
