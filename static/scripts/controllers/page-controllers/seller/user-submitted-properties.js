(function () {
  "use strict";
  console.log("UserSubmittedPropertyCtrl", UserSubmittedPropertyCtrl);
  angular
  .module("app", ["summernote", "ui.bootstrap"])
  .controller("UserSubmittedPropertyCtrl", UserSubmittedPropertyCtrl);

  UserSubmittedPropertyCtrl.$inject = [
    "$scope",
    "$http",
    "$rootScope",
    "$location",
  ...
  function UserSubmittedPropertyCtrl($scope, $http, $rootScope) {
    var vm = $scope;
    vm.search = "";
    ...
    vm.propertyStatusFilter = "";
    vm.propertyStatusFilterToSend = {};

    vm.usp = {};
    vm.usp.updatedBy = localStorage.getItem("user");
    vm.savingNotesFor = {};
    vm.userHistory = [];
    vm.userHistoryData = {};
    ...
    vm.getFilterToSend = () => {
      ...
      if (vm.userFilter && vm.userFilter !== ""){
        vm.userFilterToSend = { user: vm.userFilter}
      } else {
        vm.userFilterToSend = {};
      }

      if (vm.cityFilter){
        vm.cityFilterToSend = { city: vm.cityFilter}
      } else {
        vm.cityFilterToSend = {};
      }

      if (vm.propertyStatusFilter && vm.propertyStatusFilter !== "-"){
        vm.propertyStatusFilterToSend = { status: vm.propertyStatusFilter}
      } else {
        vm.propertyStatusFilterToSend = {};
      }

      let filterToSend = { ...vm.statusFilterToSend, ...vm.userFilterToSend, ...vm.cityFilterToSend, ...vm.propertyStatusFilterToSend };
      return filterToSend;
    }

    vm.filter = (page) => {
      let filterToSend = vm.getFilterToSend();
    ...
    vm.$watch("cityFilter", function (newValue, oldValue) {
      if (newValue != oldValue) {
        let filterToSend = vm.getFilterToSend();
        vm.getAll(1, vm.sortBy, vm.sortForm, filterToSend);
      }
    });
    ...
    vm.previousPaginationArray = function () {
      ...
      vm.getAll(vm.mainArray[vm.currentArrayIndex][0], vm.sortBy, vm.sortForm, vm.getFilterToSend());
    };
    ...
    vm.showNotesModal = (user) => {
      ...
      vm.usp.updatedBy = localStorage.getItem("user");
      if (user.history[0]) {
        vm.usp.status = user.latestStatus ? user.latestStatus : user.history[0] ? user.history[0].status : "";
        vm.usp.subStatus = user.latestSubStatus ? user.latestSubStatus : user.history[0]
          ? user.history[0].subStatus
          : "";
        vm.usp.whatsappNumber = user.history[0]
        ...
        vm.usp.whatsappNumber = user.username;
        vm.usp.status = user.latestStatus ? user.latestStatus : '';
        vm.usp.subStatus = user.latestSubStatus ? user.latestSubStatus : "";
        vm.phoneCode = user.country.code;
        vm.phoneCodeName = user.country.name;
        vm.phoneCodeSearchText = "";
      }
      console.log(vm.usp);
    };
    vm.saveNotes = async () => {
      ...
      if (vm.subStatuses[vm.usp.status] && 
        !vm.subStatuses[vm.usp.status].includes(vm.usp.subStatus)
      ) {
      ...
      let phoneNumber = await $.ajax({
        ...
        , type: "post"
        , data: {number:vm.phoneCode+vm.usp.whatsappNumber}
      });
      if (!phoneNumber.valid){
        ...
        vm.usp.whatsappNumber = phoneNumber.valid
      }
      let country = {code:vm.phoneCode, name:vm.phoneCodeName};
      $http
        .post("api/usp/addNotes", {
          data: { update: { ...vm.usp, country }, ...vm.savingNotesFor },
        })
        .then(function (res) {
          console.log("res :", res);
          if (res.data.success) {
            ...
            vm.usp = {};
            let filterToSend = vm.getFilterToSend()
            vm.getAll(vm.pageIndex, vm.sortBy, vm.sortForm, filterToSend);
            alert("Added");
    ...
    vm.showHistoryModal = (user) => {
      ...
      console.log({uhd: vm.userHistoryData});
    };
    // get all list
      ...
      $http
        ...
        .then(function (res) {
          ...
          if (res.data.success) {
            ...
            vm.allUsers = res.data.allCustomers;
            ...
            res.data.docs.forEach((user) => {
              user.edit = false; //edit the city mapping
              if (user.price && !isNaN(parseInt(user.price))) {
                user.price = parseInt(user.price).toLocaleString('en-IN');
              }
              if (user.size && !isNaN(parseInt(user.size))) {
                user.size = parseInt(user.size).toLocaleString('en-IN');
              }
              if (user.history) {
    ...
    vm.getCities = function (val) {
      return $http
        ...
        .then(function (response) {
          const toSend = response.data.results.map(function (item) {
            return item.name;
          });
          toSend.push("Other");
          console.log({toSend})
      ...
      $http
        ...
        .then(function (res) {
          ...
          if (res.data.success) {
            ...
            vm.getAll(vm.pageIndex, vm.sortBy, vm.sortForm, vm.getFilterToSend());
            console.log("Saved!");
          } else {
    ...
    vm.changeSort = function (sortBy, sortForm) {
      ...
      let filterToSend = vm.getFilterToSend()
      vm.getAll(1, vm.sortBy, vm.sortForm, filterToSend, vm.getFilterToSend());
      if (vm.sortForm === -1) {
    ...
    vm.humanifyString = (str) => {
      if (str.includes("_")) {
        ...
        str = str.split("Lead");
        str = str[0] + " Lead"
      } else if(str.includes("Qualified")){
        str = str.split("Qualified");
        str = str[0] + " Qualified"
      } else if(str.includes("Stage")){
        str = str.split("Stage");
        str = str[0] + " Stage"
      } else {
        str = str;
