(function () {
  "use strict";
  console.log("ConsultationCtrl", ConsultationCtrl);
  angular.module("app").controller("ConsultationCtrl", ConsultationCtrl);
  ...
  function ConsultationCtrl($scope, $http, $rootScope) {
    var vm = $scope;
    vm.search = "";
    ...
    vm.consult = {};
    vm.consult.updatedBy = localStorage.getItem("user");
    vm.savingNotesFor = {};
    vm.userHistory = [];
    vm.userHistoryData = {};
    vm.subStatuses = {
    ...
    vm.showNotesModal = (user) => {
        ...
        vm.consult.updatedBy = localStorage.getItem("user");
        if (user.history[0]) {
          vm.consult.status = user.history[0] ? user.history[0].status : "";
          vm.consult.subStatus = user.history[0] ? user.history[0].subStatus : "";
          vm.consult.whatsappNumber = user.history[0] ? user.history[0].whatsappNumber : "";
          vm.phoneCode = user.history[0].country.code;
          vm.phoneCodeName = user.history[0].country.name;
          vm.phoneCodeSearchText = "";
        } else{
          vm.consult.whatsappNumber = user.phone;
        }
    ...
    vm.saveNotes = async () => {
      ...
      if (vm.consult.status && !vm.subStatuses[vm.consult.status].includes(vm.consult.subStatus)){
      ...
      let phoneNumber = await $.ajax({
        ...
        , type: "post"
        , data: {number:vm.phoneCode+vm.consult.whatsappNumber}
      });
      if (!phoneNumber.valid){
        ...
        vm.consult.whatsappNumber = phoneNumber.valid
      }

      let country = {code: vm.phoneCode, name:vm.phoneCodeName};

      $http
        .post("api/consult/addNotes", {
          data: { update: { ...vm.consult, country }, ...vm.savingNotesFor },
        })
        .then(function (res) {
          console.log("res :", res);
          if (res.data.success) {
      ...
      vm.showHistoryModal = (user) => {
        $("#showConsultHistoryModal").modal("show");
        vm.userHistory = user.history;
        vm.userHistoryData = user;
        console.log({userHistoryData: vm.userHistoryData})
      };
