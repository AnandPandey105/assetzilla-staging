(function () {
  "use strict";
  // console.log("BookingProperty", BookingPropertyCtrl);
  angular.module("app").controller("BookingPropertyCtrl", BookingPropertyCtrl);

  BookingPropertyCtrl.$inject = [
    "$scope",
    "$http",
    "$rootScope",
    "$location",
  ...
  function BookingPropertyCtrl($scope, $http, $rootScope) {
    var vm = $scope;
    vm.search = "";
    ...
    vm.enquiry = {};
    vm.enquiry.updatedBy = localStorage.getItem("user");
    vm.savingNotesFor = {};
    vm.userHistory = [];
    vm.userHistoryData = {};
    vm.subStatuses = {
    ...
    vm.showNotesModal = (user) => {
      ...
      vm.enquiry.updatedBy = localStorage.getItem("user");
      if (user.history[0]) {
        vm.enquiry.status = user.history[0] ? user.history[0].status : "";
        vm.enquiry.subStatus = user.history[0] ? user.history[0].subStatus : "";
        vm.enquiry.whatsappNumber = user.history[0]
          ? user.history[0].whatsappNumber
          : "";
        vm.phoneCode = user.history[0].country.code;
        vm.phoneCodeName = user.history[0].country.name;
        vm.phoneCodeSearchText = "";
      } else {
        vm.enquiry.whatsappNumber = user.number;
      }
      console.log(vm.enquiry);
    };

    vm.saveNotes = async () => {
      $("#submit-notes").hide();
      $("#submitting-notes").fadeIn();
      if (vm.subStatuses[vm.enquiry.status] && !vm.subStatuses[vm.enquiry.status].includes(vm.enquiry.subStatus)) {
      ...
      let phoneNumber = await $.ajax({
        ...
        , type: "post"
        , data: {number:vm.phoneCode+vm.enquiry.whatsappNumber}
      });
      if (!phoneNumber.valid){
        ...
        vm.enquiry.whatsappNumber = phoneNumber.valid
      }
      let country = {code:vm.phoneCode, name:vm.phoneCodeName}
      console.log(country)
      console.log({data: { update: { ...vm.enquiry, country }, ...vm.savingNotesFor },})
      $http
        .post("api/booking_property/addNotes", {
          data: { update: { ...vm.enquiry, country:{code:vm.phoneCode, name:vm.phoneCodeName} }, ...vm.savingNotesFor },
        })
        .then(function (res) {
          console.log("res :", res);
          if (res.data.success) {
    ...
    vm.showHistoryModal = (user) => {
      $("#showEnquiryHistoryModal").modal("show");
    ...
    vm.getAll = function (currentPageIndex, sortBy, sortForm, filter) {
      ...
      $http
        .post("api/booking_property/get/bookedproperties", {
          param: {
            search: vm.search,
          },
          sortBy: sortBy,
