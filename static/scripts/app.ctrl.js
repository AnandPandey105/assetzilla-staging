...
(function () {
  ...
  function AppCtrl($scope, $localStorage, $location, $rootScope, $anchorScroll, $timeout, $window, mainService) {
    var vm = $scope;
    vm.isIE = isIE();
    vm.isSmart = isSmart();
    ...
    function openPage() {
      ...
      $location.hash('');
      // hide open menu
      var dlgElem = angular.element("#aside");
      if (dlgElem) {
        dlgElem.modal("hide");
      }

      $('body').removeClass('modal-open').find('.modal-backdrop').remove();
      var newdlgElem=angular.element(".navbar-toggleable-sm");
      if (newdlgElem) {
        newdlgElem.collapse('hide');
      }
    ...
    $rootScope.createError = function (error) {
    ...
    $rootScope.createSuccess = function (success) {
